...
public:
  ...
  : Node("bimanual_panel_roll_demo_node")
  {
    ...
    right_goal_pose_topic_ =
      this->declare_parameter("right_goal_pose_topic", std::string("/r_goal_pose"));
    left_goal_pose_topic_ =
      this->declare_parameter("left_goal_pose_topic", std::string("/l_goal_pose"));
    ...
    right_goal_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(
      right_goal_pose_topic_, rclcpp::QoS(rclcpp::KeepLast(1)).best_effort());
    left_goal_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(
      left_goal_pose_topic_, rclcpp::QoS(rclcpp::KeepLast(1)).best_effort());
    virtual_object_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(
...
private:
  ...
  void timerCallback()
  {
    ...
    if (phase_ == Phase::MOVE_TO_PANEL) {
      const double alpha = elapsed(now) / std::max(1e-6, move_duration_);
      publishHandGoals(blendPoses(right_move_start_pose_, right_target_pose_, alpha),
        blendPoses(left_move_start_pose_, left_target_pose_, alpha), now);
      if (alpha >= 1.0) {
        phase_start_time_ = now;
    ...
    if (phase_ == Phase::HOLD_PANEL) {
      publishHandGoals(right_target_pose_, left_target_pose_, now);
    ...
    if (phase_ == Phase::CAPTURE_GRASP) {
      publishVirtualObjectGoal(currentVirtualObjectPose(), now);
    ...
    publishVirtualObjectGoal(makeRolledObjectGoal(now), now);
  }
  ...
  {
    right_goal_pub_->publish(makePoseMsg(frame_id_, stamp, right_pose));
    left_goal_pub_->publish(makePoseMsg(frame_id_, stamp, left_pose));
  }

  void publishVirtualObjectGoal(const Eigen::Affine3d & pose, const rclcpp::Time & stamp) const
  {
    virtual_object_pub_->publish(makePoseMsg(frame_id_, stamp, pose));
  }
  ...
  rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr right_goal_pub_;
  rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr left_goal_pub_;
  rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr virtual_object_pub_;
