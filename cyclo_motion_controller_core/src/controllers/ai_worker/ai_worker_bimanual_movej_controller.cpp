...
: cyclo_motion_controller::optimization::QPBase(), robot_data_(robot_data), dt_(dt)
{
  ...
  w_joint_tracking_.setOnes(joint_dof_);
...
void AIWorkerBimanualMoveJController::setDesiredJointVel(const Eigen::VectorXd & qdot_desired)
{
...
void AIWorkerBimanualMoveJController::setWeight(
  const Eigen::VectorXd & w_tracking,
  const Eigen::VectorXd & w_damping)
{
  if (w_tracking.size() == joint_dof_) {
    w_joint_tracking_ = w_tracking;
  }
...
void AIWorkerBimanualMoveJController::setControllerParams(
...
void AIWorkerBimanualMoveJController::setConstraintLinks(
...
void AIWorkerBimanualMoveJController::setRigidGraspPoseConstraint(
...
bool AIWorkerBimanualMoveJController::getOptJointVel(Eigen::VectorXd & opt_qdot)
{
  Eigen::MatrixXd sol;
  if (!solveQP(sol)) {
    opt_qdot.setZero();
...
Eigen::VectorXd AIWorkerBimanualMoveJController::projectDesiredVelocityToGraspManifold(
  const Eigen::VectorXd & qdot_desired) const
{
  if (!rigid_grasp_active_) {
    return qdot_desired;
  }
  ...
  const Eigen::MatrixXd grasp_jacobian = jl - transform * jr;

  constexpr double kDamping = 1e-8;
  const Eigen::MatrixXd regularized =
    grasp_jacobian * grasp_jacobian.transpose() +
    kDamping * Eigen::MatrixXd::Identity(grasp_jacobian.rows(), grasp_jacobian.rows());
  const Eigen::VectorXd correction =
    grasp_jacobian.transpose() * regularized.ldlt().solve(grasp_jacobian * qdot_desired);
  return qdot_desired - correction;
}

void AIWorkerBimanualMoveJController::setCost()
{
  P_ds_.setZero(nx_, nx_);
  q_ds_.setZero(nx_);

  const Eigen::VectorXd qdot_desired = projectDesiredVelocityToGraspManifold(qdot_desired_);
  P_ds_.block(
    si_index_.qdot_start, si_index_.qdot_start,
    si_index_.qdot_size, si_index_.qdot_size) +=
    2.0 * w_joint_tracking_.asDiagonal();
  q_ds_.segment(si_index_.qdot_start, si_index_.qdot_size) +=
    -2.0 * w_joint_tracking_.asDiagonal() * qdot_desired;
...
void AIWorkerBimanualMoveJController::setBoundConstraint()
{
...
void AIWorkerBimanualMoveJController::setIneqConstraint()
{
...
void AIWorkerBimanualMoveJController::setEqConstraint()
{
